...
export class Player extends Entity {
  ...
  constructor(scene, x, y, characterClass = 'warrior') {
    ...
    // 융합 스킬 쿨다운 저장
    this.fusionSkillCooldowns = new Map();
  ...
  updateAvailableSkills() {
    ...
    if (this.characterClass === 'fusionist') {
      ...
      // fusionist는 전직 후 바로 모든 기본 스킬 사용 가능
      // 스킬 1 (속성 탄환 - 전직 후 바로 사용 가능)
      ...
      // 스킬 2 (마력 장벽 - 전직 후 바로 사용 가능)
      if (this.skillData['2'] && !this.skills['2']) {
      ...
      // 스킬 3 (불안정한 파동 - 전직 후 바로 사용 가능)
      if (this.skillData['3'] && !this.skills['3']) {
      ...
      // 스킬 4 (원소 융합 - 전직 후 바로 사용 가능)
      if (this.skillData['R'] && !this.skills['R']) {
  ...
  changeSkillSlot(slotKey, newSkillData) {
    ...
    // 융합 스킬은 player.skills에 추가
    if (newSkillData.id.startsWith('fusion_')) {
      // 융합 스킬은 Skill 인스턴스로 변환
      const skillInstance = this.createSkillInstance(newSkillData);
      if (!skillInstance) {
        console.error('융합 스킬 인스턴스 생성 실패:', newSkillData);
        return false;
      }
      ...
      console.log(`스킬 슬롯 ${slotKey}에 융합 스킬 ${newSkillData.name} 설정됨`);
  ...
  /**
   * 안전하게 스킬 목록 가져오기 (FusionUI용)
   */
  getAvailableSkillsForFusion() {
    const skills = [];
        ...
        Object.values(this.skills).forEach(skill => {
          if (skill && skill.name && skill.id) {
            skills.push({
              ...
              type: skill.type || 'melee' // 스킬 타입 포함
            });
          }
        });
      } catch (error) {
        console.warn('[Player] 스킬 목록 가져오기 중 오류:', error);
      }
    }
    return skills;
  }
   ...
   */
  useSkill(slotKey) {
    console.log('[Player] useSkill 호출됨, slotKey:', slotKey, 'characterClass:', this.characterClass);

    // 융합술사인 경우 FusionSkillUI에서 선택된 스킬 사용
    if (this.characterClass === 'fusionist' && this.scene.fusionSkillUI) {
      const selectedSkill = this.scene.fusionSkillUI.getSelectedSkill(slotKey);
      if (selectedSkill) {
        console.log('[Player] 선택된 스킬 사용:', selectedSkill.name, selectedSkill.id);
        this.executeSkillById(selectedSkill.id);
        return;
      }
    }

    // 일반적인 경우: 슬롯에 있는 스킬 사용
    const skill = this.skills[slotKey];
    if (skill && typeof skill.use === 'function') {
      console.log('[Player] 슬롯 스킬 사용:', skill.name, slotKey);
      skill.use(this);
    } else {
      console.warn('[Player] 스킬이 없거나 실행할 수 없음, slotKey:', slotKey, 'skill:', skill);
    }
  }
   ...
   * 스킬 ID로 스킬 실행
   */
  executeSkillById(skillId) {
    console.log('[Player] executeSkillById 호출됨, skillId:', skillId, 'characterClass:', this.characterClass);

    // 융합술사 스킬인 경우 FusionistSkills에서 실행
    if (this.characterClass === 'fusionist' && skillId.startsWith('fusionist_')) {
      console.log('[Player] 융합술사 스킬 실행 시도:', skillId);
      return this.executeFusionistSkill(skillId);
    }

    // 융합 스킬인 경우
    if (skillId.startsWith('fusion_')) {
      console.log('[Player] 융합 스킬 실행 시도:', skillId);
      return this.executeFusionSkill(skillId);
    }

    // 일반 스킬인 경우
    const skill = this.findSkillById(skillId);
    if (skill && typeof skill.use === 'function') {
      console.log('[Player] 일반 스킬 사용 시도:', skill.name, 'MP:', this.stats.mp, 'mpCost:', skill.mpCost);
      const result = skill.use(this);
      console.log('[Player] 일반 스킬 사용 결과:', result, '남은 MP:', this.stats.mp);
      return result;
    } else {
      console.warn('[Player] 스킬을 찾을 수 없음, skillId:', skillId, 'skill:', skill);
    }
  }
   ...
   * 융합술사 스킬 실행
   */
  executeFusionistSkill(skillId) {
    console.log('[Player] executeFusionistSkill 호출됨, skillId:', skillId);
    ...
    try {
      ...
      if (!skillData) {
        console.error('[Player] 스킬 데이터를 찾을 수 없음:', skillId);
        return false;
      }

      console.log('[Player] 스킬 데이터:', skillData);

      // createSkill을 사용해서 스킬 객체 생성 후 실행
      const skill = createSkill(skillData, 'fusionist');
      if (skill && typeof skill.use === 'function') {
        console.log('[Player] 융합술사 스킬 사용 시도:', skill.name, 'MP:', this.stats.mp, 'mpCost:', skill.mpCost);
        const result = skill.use(this);
        console.log('[Player] 융합술사 스킬 사용 결과:', result, '남은 MP:', this.stats.mp);
        return result;
      } else {
        console.error('[Player] 스킬 사용 메서드가 없음:', skillId);
        return false;
      }
    } catch (error) {
      console.error('[Player] 융합술사 스킬 실행 중 오류:', error);
      return false;
    }
  }
   ...
   * 융합 스킬 실행
   */
  executeFusionSkill(skillId) {
    console.log('[Player] executeFusionSkill 호출됨, skillId:', skillId);

    try {
      const FusionistSkills = require('./skills/FusionistSkills.js');
      const result = FusionistSkills.executeFusionSkill(skillId, this);
      console.log('[Player] 융합 스킬 사용 결과:', result, '남은 MP:', this.stats.mp);
      return result;
    } catch (error) {
      console.error('[Player] 융합 스킬 실행 중 오류:', error);
      return false;
    }
  }
