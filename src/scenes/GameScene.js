...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    // FusionUI 초기화 (융합술사 전용) - Player 초기화 완료 후
    this.time.delayedCall(1000, () => {
      if (this.player && this.player.characterClass && this.player.skills) {
        this.fusionUI = new FusionUI(this, this.player);
        this.fusionSkillUI = new FusionSkillUI(this, this.player);
        console.log('[GameScene] FusionUI 초기화 완료, 플레이어 클래스:', this.player.characterClass, '스킬 수:', Object.keys(this.player.skills).length);
      } else {
        console.warn('[GameScene] FusionUI 초기화 실패: 플레이어가 준비되지 않음');
        // 2초 후 다시 시도
        this.time.delayedCall(2000, () => {
          if (this.player && this.player.characterClass && this.player.skills) {
            this.fusionUI = new FusionUI(this, this.player);
            this.fusionSkillUI = new FusionSkillUI(this, this.player);
            console.log('[GameScene] FusionUI 초기화 재시도 성공');
          }
        });
      }
    });
  ...
  setupInput() {
    ...
    this.input.keyboard.on('keydown-ESC', () => {
      ...
      if (uiScene) {
        ...
        // FusionUI가 열려있으면 닫기
        if (this.fusionUI && this.fusionUI.isOpen) {
          this.fusionUI.hide();
          console.log('[GameScene] ESC - 융합창 닫기');
          return;
        }

        // FusionSkillUI가 열려있으면 닫기
        if (this.fusionSkillUI && this.fusionSkillUI.isOpen) {
          this.fusionSkillUI.hide();
          console.log('[GameScene] ESC - 스킬창 닫기');
          return;
        }
    ...
    this.input.keyboard.on('keydown-R', () => {
      if (this.player && this.player.characterClass === 'fusionist' && this.fusionUI && typeof this.fusionUI.toggle === 'function') {
        this.fusionUI.toggle();
        console.log('[GameScene] R키 - 융합술사 융합 UI 토글');
      } else if (this.player && this.player.characterClass === 'fusionist' && !this.fusionUI) {
        console.log('[GameScene] R키 - FusionUI가 아직 초기화되지 않았습니다');
      }
    });

    // K 키 - 융합술사 전용 스킬 관리 UI
    this.input.keyboard.on('keydown-K', () => {
      console.log('[GameScene] K키 이벤트 실행 - fusionUI.isOpen:', this.fusionUI?.isOpen, 'hasSkillSelectorOpen:', this.fusionUI?.hasSkillSelectorOpen?.());

      // FusionUI가 열려있으면 우선 FusionUI 처리
      if (this.fusionUI && this.fusionUI.isOpen) {
        // 스킬 선택기가 열려있으면 스킬 선택기만 닫기 (X 버튼처럼)
        if (this.fusionUI.hasSkillSelectorOpen()) {
          console.log('[GameScene] K키 - 스킬 선택기 닫기 실행');
          this.fusionUI.closeSkillSelector();
          console.log('[GameScene] K키 - 스킬 선택기 닫기 완료');
        } else {
          // 스킬 선택기가 없으면 FusionUI만 닫기
          this.fusionUI.hide();
          console.log('[GameScene] K키 - FusionUI 닫기');
        }
        return;
      }

      // FusionUI가 닫혀있으면 FusionSkillUI 토글은 UIScene에서 처리하므로 여기서는 아무것도 하지 않음
      console.log('[GameScene] K키 - FusionUI가 닫혀있음, FusionSkillUI 토글은 UIScene에서 처리');
    });

    // Q, W, E 키 - 융합술사 전용 선택 스킬 사용
    this.input.keyboard.on('keydown-Q', () => {
      if (this.player && this.player.characterClass === 'fusionist' && this.fusionSkillUI) {
        const selectedSkill = this.fusionSkillUI.getSelectedSkill('1');
        if (selectedSkill) {
          this.player.executeSkillById(selectedSkill.id);
          console.log('[GameScene] Q키 - 선택된 스킬 사용:', selectedSkill.name);
        } else {
          console.log('[GameScene] Q키 - 선택된 스킬이 없음');
        }
      }
    });
    ...
    this.input.keyboard.on('keydown-W', () => {
      if (this.player && this.player.characterClass === 'fusionist' && this.fusionSkillUI) {
        const selectedSkill = this.fusionSkillUI.getSelectedSkill('2');
        if (selectedSkill) {
          this.player.executeSkillById(selectedSkill.id);
          console.log('[GameScene] W키 - 선택된 스킬 사용:', selectedSkill.name);
        } else {
          console.log('[GameScene] W키 - 선택된 스킬이 없음');
        }
      }
    });
    ...
    this.input.keyboard.on('keydown-E', () => {
      if (this.player && this.player.characterClass === 'fusionist' && this.fusionSkillUI) {
        const selectedSkill = this.fusionSkillUI.getSelectedSkill('3');
        if (selectedSkill) {
          this.player.executeSkillById(selectedSkill.id);
          console.log('[GameScene] E키 - 선택된 스킬 사용:', selectedSkill.name);
        } else {
          console.log('[GameScene] E키 - 선택된 스킬이 없음');
        }
      }
    });
  }
  ...
  update(time, delta) {
    ...
    if (this.fusionSkillUI) {
      this.fusionSkillUI.update();
    }
    if (this.fusionUI) {
      this.fusionUI.update();
    }
