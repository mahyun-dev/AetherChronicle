...
export class DataManager {
   ...
   * 융합 레시피 데이터 로드
   */
  async loadFusionRecipes() {
    try {
      const response = await fetch('assets/data/fusion_recipes.json');
      const data = await response.json();
      
      this.fusionRecipes = data.recipes || [];
      
      console.log(`[DataManager] 융합 레시피 데이터 로드: ${this.fusionRecipes.length}개`);
    } catch (error) {
      console.warn('[DataManager] 융합 레시피 데이터 로드 실패:', error);
      this.fusionRecipes = [];
    }
  }
